...
export function isPreschoolEvalMode(value: unknown): value is 'LEVEL' | 'FREQUENCY' {
  return value === PRESCHOOL_EVAL_LEVEL || value === PRESCHOOL_EVAL_FREQUENCY;
}
...
export function isYearEndDecision(value: string): boolean {
  return YEAR_END_DECISIONS.some((d) => d.value === value);
}
